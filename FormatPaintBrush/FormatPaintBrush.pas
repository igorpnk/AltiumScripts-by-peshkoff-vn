...
Begin
    ...
    Try
        ...
        Registry.OpenKey('Software\AltiumScripts\FormatPaintBrush',true);

        if Registry.ValueExists('LayerCopy') then    cbCopyLayerInfo.Checked := Registry.ReadBool('LayerCopy');
...
Begin
    ...
    Try
        ...
        Registry.WriteBool('LayerCopy',cbCopyLayerInfo.Checked);
...
function FormatPaintBrush(reply : boolean);
var
...
Begin
   ...
   Begin
      ...
      Begin
         //
         if not(reply) then SourcePrim := PCBBoard.GetObjectAtCursor(ASetOfObjects, AllLayers,'Choose Source Primitive');
         //ShowMessage(floattostr(x));
         //SourcePrim := PCBBoard.GetObjectAtXYAskUserIfAmbiguous(x,y,ASetOfObjects,AllLayers,eEditAction_Select);
         if SourcePrim = nil then exit
         else
         begin
              boolLayerCopy := cbCopyLayerInfo.Checked;
...
procedure FormatPaintBrushStart;
begin
    RegistryRead;
    FormatPaintBrush(false);
end;

procedure FormatPaintBrushReply;
begin
    RegistryRead;
    FormatPaintBrush(true);
end;

procedure FormatPaintBrushSet;

begin
    RegistryRead;
    frmFormatPaintBrush.Show;
end;

procedure TfrmFormatPaintBrush.btnCancelClick(Sender: TObject);
begin
    frmFormatPaintBrush.Close;
end;

procedure TfrmFormatPaintBrush.btnOKClick(Sender: TObject);
begin
    RegistryWrite;
    frmFormatPaintBrush.Close;
end;
