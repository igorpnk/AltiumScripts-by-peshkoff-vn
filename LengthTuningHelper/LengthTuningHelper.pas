...
Begin
    ...
    Try
        ...
        Registry.OpenKey('Software\AltiumScripts\LengthTuningHelper',true);

        if Registry.ValueExists('RefDes')    then    RegRefDes := Registry.ReadString('RefDes');
        if Registry.ValueExists('FileName')  then    FileName := Registry.ReadString('FileName');
        if Registry.ValueExists('Delay')     then    EditDelay.Text: = Registry.ReadString('Delay');
        if Registry.ValueExists('NetClass')  then    RegNetClass := Registry.ReadString('NetClass');
        if Registry.ValueExists('ViaLength') then    CheckBoxViaLength.Checked := Registry.ReadBool('ViaLength');
...
Begin
    ...
    Try
        ...
        Registry.WriteString('RefDes',   ComboBoxIC.Text);
        Registry.WriteString('FileName', FileName);
        Registry.WriteString('Delay', FloatToStr(EditDelay.Text));
        Registry.WriteString('NetClass', ComboBoxNetClass.Text);
        Registry.WriteBool('ViaLength',  CheckBoxViaLength.Checked);
...
begin
   ...
   begin
      Line := PkgFile[i];
      Line := StringReplace(Line, #9, #32, mkset(rfReplaceAll));    //replace TAB to space
      Line := RemoveMultipleSpaces(Line);

      if ((AnsiCompareText(GetToken(Line, 1, ' '),'pin') = 0) or
          (AnsiCompareText(GetToken(Line, 1, ' '),'pkgpin') = 0)) and
          (AnsiCompareText(GetToken(Line, 9, ' '),'N.A.') <> 0) then
      begin
         // pin info
         PinDes := GetToken(Line, 3, ' ');
         Len    := GetToken(Line, 9, ' ');

         if  ParsedFile[0] = 'Units;ps' then
             begin
             Len := FloatToStr((StrToFloat(Len) + StrToFloat(GetToken(Line, 10, ' ')))/2); //Mid for Max Delay and Min Delay
             end;

//         if (AnsiCompareText(Len,'N.A.') <> 0) then          //If N.A. then not included to file
             ParsedFile.Add(PinDes + ';' + Len);
      end
      else
      begin
          // finding units here
          if (AnsiCompareText(GetToken(Line, 9, ' '),'tracelength') = 0) or
             ((AnsiCompareText(GetToken(Line, 9, ' '),'Min') = 0) and (AnsiCompareText(GetToken(Line, 10, ' '),'Delay') = 0)) then
          begin
             Inc(i);
             Line := PkgFile[i];
...
begin
   ...
   else if (StrUnits = 'ps')   or (StrUnits = 'PS')   then Units := mmsToCoord(1/StrToFloat(FloatToStr(EditDelay.Text)))
...
begin
   ...
   begin
      ...
      Track.Width := MMsToCoord(0.15);//Rule.MinWidth(eTopLayer);
...
begin
   RegRefDes:='';
   RegNetClass:='';
   RegistryRead; //Read data from registry;
   ...
   While comp <> NIl Do
   Begin
        if (pos('BGA', comp.Pattern) <> 0) then
        begin
            ComboBoxIC.Items.AddObject(comp.Name.Text + ' - ' + comp.Comment.Text, comp);
            //Compare RegRefDes and comp.Name.Text
            if RegRefDes = (comp.Name.Text + ' - ' + comp.Comment.Text) then
            begin
                ComboBoxIC.ItemIndex:=ComboBoxIC.Items.IndexOf(RegRefDes);
                //ComboBoxIC.Items();
            end;
        end;
   ...
   Begin
       If ((NetClass.MemberKind = eClassMemberKind_Net) and (NetClass.Name <> 'All Nets')) Then
        ...
        //Compare RegNetClass and NetClass.Name
        if RegNetClass = NetClass.Name then
        begin
            ComboBoxNetClass.ItemIndex:=ComboBoxNetClass.Items.IndexOf(RegNetClass);
            //ComboBoxIC.Items();
        end;
...
begin
   openFileDialog.InitialDir := ExtractFilePath(FileName);
...
begin
     ...
     EditDelayStr := EditDelay.Text;
     j := 1;
     while j <= Length(EditDelayStr) do
         begin
         ECode := Ord(EditDelayStr[j]);
         if ((ECode < 48) or (ECode > 57)) and (ECode <> 44) and (ECode <> 46) then
            begin
                 ShowMessage('Please enter valid number to fileld "Delay"');
                 exit;
            end;
         Inc(j);
         end; //while
...
procedure TFormLengthTuning.FormLengthTuningResize(Sender: TObject);
begin
     ComboBoxIC.Width := FormLengthTuning.Width - 112;
     ComboBoxNetClass.Width := FormLengthTuning.Width - 112;
     ButtonOK.Left := (FormLengthTuning.Width div 2) - 82;
     ButtonCancel.Left := FormLengthTuning.Width div 2 + 6;
end;
